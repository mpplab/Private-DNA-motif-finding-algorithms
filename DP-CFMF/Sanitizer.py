...
def equivalence(ngrams_set,input_file,output_file):
    fp = open(input_file)
    lines = fp.readlines()
    grams = []
    sups = []
    ...
    for i in range(len(grams)):
        ...
        if i == len(grams) - 1:
            ...
            for j in range(i+1,len(grams)):
                if len(grams[j])<= len(grams[i]):
                    continue
                else:
                    if((''.join(grams[j])).startswith((''.join(grams[i])))== True):
                        continue
                    else:
                        if(''.join(grams[j]).find(''.join(grams[i]))==-1):
                            continue
                        else:
                            if(sups[i] == sups[j] ):
                                flag = True
                                eCount += 1
                                print str(eCount) +".Equal relationship: " + ''.join(grams[i]) + " and " + ''.join(grams[j]) + ", support: " + sups[i]
                                break
                            else:
                                flag = False
        if flag == False:
            ngrams_set[strToSeq(grams[i],dec=1)] = float(sups[i].strip())
  
    file = open(output_file,'w')
    file.write(str(len(ngrams_set))+"\n")
    pbar = MyProgressBar('Dumping ',len(ngrams_set))
    i = 0
    for gram in sorted(ngrams_set.keys(),key=cmp_to_key(compare_grams)):
        file.write("%s : %f\n"%(" ".join(map(lambda x:str(ord(x)+1),gram)),ngrams_set[gram]))
        i+=1
        pbar.update(i)
    pbar.finish()
    file.close()
...
def Equivalence(ngram_set, input_file, output_file):
    if os.path.isfile(output_file) == False :
        ngram_set = equivalence(ngram_set, input_file, output_file)
    else:
            ngram_set.clear()
            f = open(output_file)
            ngram_num =int(f.readline().strip())
            print"Clos-NGrams:",ngram_num
            print"Loading Clos-NGram file(%s,N=%d)..."%(output_file,ngram_set.N_max)
            pbar = MyProgressBar('Loading dump',ngram_num)
            for (line_num,line) in enumerate(f):
                parts = line.strip().partition(':')
                tokens = parts[0].strip().split()
                strs=''.join(tokens)
                ngram_set[strToSeq(strs,dec=1)] = float(parts[2].strip())
                max_item = max(map(int,strs)) - 1
                if ngram_set.alphabet_size < max_item:
                    ngram_set.alphabet_size = max_item
                pbar.update(line_num + 1)
            pbar.finish()
            ngram_set.TERM = ngram_set.alphabet_size
            print"Alphabet size:",ngram_set.alphabet_size
