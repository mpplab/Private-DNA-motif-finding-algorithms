...
#    lUp = 8/root/Project/Mao/Diff-FSPM-Pro/Diff-FSPM/data
    ...
    true_output_fileName = true_outdataset_name % (n_max, topN, lLeft, lUp, deta) + ".dat"
    noise_output_fileName = noise_outdataset_name % (n_max, topN, lLeft, lUp, deta, epsilon) + ".dat"
    ...
    #FileCheck(dataset_gene + ".dat", dataset + ".dat")
    
    ngram_set = NGramSet(int(l_max), n_max)
    ngram_set.load_dataset(input_dir + dataset_name + ".dat", output_dir + dataset_name + "-original-" + str(n_max) + "grams.dat")
    ...
    factory.ngramset.dump(outdataset_name_info % (n_max, topN, lLeft, lUp, deta, epsilon) + "-extended.dat")

    # Generating dataset
    factory.reconstruct(outdataset_name_info % (n_max, topN, lLeft, lUp, deta, epsilon) + "-reconstructed.dat")
    ...
    Sanitizer.calculate2(outdataset_name_info % (n_max, topN, lLeft, lUp, deta, epsilon) + "-extended.dat",lLeft,lUp,deta,noise_output_fileName,topN)
    ...
    ngram_set = NGramSet_true(l_max, n_max2, min_sup)
    ngram_set.load_dataset(input_dir + dataset_name + ".dat", true_outdataset_name % (lUp, topN, lLeft, lUp, deta) + "-original-" + str(n_max) + "grams.dat")
    ...
    Sanitizer.calculate2(true_outdataset_name % (lUp, topN, lLeft, lUp, deta) + "-original-" + str(n_max) + "grams.dat",lLeft,lUp,deta,true_output_fileName,topN)
